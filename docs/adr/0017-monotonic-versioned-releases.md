...
3. A release workflow may publish only from an exact successful CI result on canonical `main` and MUST bind all release writes to that successful CI head SHA.
4. If the candidate version tag/release already exists, the workflow MUST verify the expected counterpart, exact tag target, public non-draft/non-prerelease state, and exact expected asset set. Any mismatch fails closed. An exact match exits successfully without mutation.
5. If neither candidate tag nor release exists, the workflow may create the tag at the bound release SHA and publish the corresponding GitHub Release with only the expected wheel and source distribution.
...
- The release workflow becomes reusable for later shaped versions without weakening exact-SHA, asset, or release-state verification.
